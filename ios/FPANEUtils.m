...
void AirNativeShare_FPANE_Log(FREContext context, NSString* message) {
    ...
    AirNativeShare_FPANE_DispatchEventWithInfo(context, @"LOGGING", message);
}
...
NSArray* AirNativeShare_FPANE_FREObjectToNSArrayOfUIImage(FREObject object) {
    ...
    for (NSInteger i = 0; i < arrayLength; i++) {
        ...
        UIImage *image = AirNativeShare_FPANE_FREBitmapDataToUIImage(itemRaw);
...
FREObject AirNativeShare_FPANE_CreateError(NSString* error, NSInteger* id) {
    ...
    FREObject argV[] = {
            AirNativeShare_FPANE_NSStringToFREObject(error),
            freId
    };
